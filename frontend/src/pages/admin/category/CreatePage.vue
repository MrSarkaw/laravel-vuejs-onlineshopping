...
export default {
  ...
  methods: {
    storeOrUpdate(){
      let url =this.isUpdate ? 'admin/category/'+this.$route.params.id+'?_method=put' : 'admin/category'
    ...
    deleteData(){
      ...
      if (result.isConfirmed) {
        this.$axios.delete('/admin/category/'+this.$route.params.id).then(()=>{
          this.$Swal.fire(
            'Deleted!',
  ...
  created() {
    ...
    if(id){
      this.isUpdate = true;
      this.$axios.get(`admin/category/${id}/edit`).then(({ data })=>{
